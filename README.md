...
- **PWA:** 홈 화면 설치, 독립 앱 창, 오프라인 시작 화면, 새 버전 알림
...
HTTPS 배포 주소에서 한 번 접속하면 서비스 워커가 앱 화면과 아이콘을 준비합니다. 지원하는 브라우저에서는 로그인 화면 또는 계정 설정의 **Gym Log 앱 설치** 버튼으로 설치할 수 있습니다. iPhone/iPad에서는 브라우저의 **공유 → 홈 화면에 추가**를 사용합니다. 설치하면 브라우저 주소 표시줄 없이 독립된 앱 창으로 열립니다.

앱을 한 번 불러온 뒤에는 인터넷이 없어도 시작 화면이 열립니다. **로그인, 서버 기록 불러오기와 저장에는 인터넷 연결이 필요합니다.** 이미 열린 기록과 작성 화면은 연결이 끊겨도 유지되며, 저장 실패 시 편집 화면에서 연결 후 다시 저장할 수 있습니다. 오프라인 작성 내용을 영구 보관하거나 자동 업로드하지 않으므로 저장 전 앱을 닫거나 새로고침하지 마세요. 연결 복구 시 로그인 상태를 다시 확인하고, 편집 중이 아닐 때 기록을 갱신합니다.

새 빌드를 발견하면 업데이트 알림을 표시합니다. 작성·저장 중에는 업데이트 버튼을 비활성화하고, 사용자가 업데이트를 선택하고 확인한 뒤 새로고침합니다. `나중에`를 누르면 현재 화면을 계속 사용할 수 있습니다. 여러 창을 사용 중이면 모든 창에서 기록을 저장한 뒤 업데이트하세요.

정적 빌드 파일만 사전 캐시하며 `/api` 요청과 인증·운동 기록 응답은 캐시하지 않습니다. 외부 웹 폰트는 오프라인에서 시스템 글꼴로 표시될 수 있습니다. 서비스 워커는 운영 빌드에서만 활성화되며, 개발 중에는 등록하지 않습니다.

로컬 확인:

```sh
npm run build
npx vite preview --host localhost --port 4173
```

`http://localhost:4173`에서 정적 화면과 서비스 워커를 확인할 수 있습니다. 이 Vite 미리보기는 API를 제공하지 않으므로 로그인·저장은 아래 자동 테스트 또는 HTTPS 배포 환경에서 확인하세요. 전체 운영 서버인 `npm start`는 기존과 같이 HTTPS `APP_ORIGIN` 설정이 필요합니다. 휴대폰에서 일반 HTTP LAN 주소로 접속하면 설치·서비스 워커가 제한되므로 HTTPS 주소를 사용하세요.

재현 가능한 Chromium 검증은 실제 계정·DB와 분리된 테스트 API로 실행합니다:
...
매니페스트·아이콘·브라우저 설치 요건, 오프라인 재실행, API 캐시 제외, 연결 복구 후 저장, 업데이트 대기·확인, 설치 UI와 모바일 레이아웃을 검사합니다. PNG 아이콘은 저장소에 포함돼 있으며 변경 시 `npm run icons`로 재생성할 수 있습니다.
